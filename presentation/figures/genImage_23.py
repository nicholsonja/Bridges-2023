...
def draw_frame(N, lemniscate, hypotrochoid):
    # lemniscate_points = []
    # triangle_points = []
    result_points = []
    result = lemniscate + hypotrochoid
...
def makeImage(height, width, filename):
    ...
    hypotrochoid = shape.Hypotrochoid(0, 0,  500, 100, 500)
    rose_path = shape.RoseCurve(0, 0,  250, 2, 1)
    lemniscate = shape.Lemniscate(0, 0, alpha=200)
    ...
    for offset_num in range(offsets + 1):
        ...
        tmp_lemniscate = lemniscate.rotate(-2 * theta).translate(rose_pt[0], rose_pt[1])

        frame_points = draw_frame(N, tmp_lemniscate, hypotrochoid)
