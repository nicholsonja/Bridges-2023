...
def makeImage(height, width, filename):
    ...
    circle = shape.Circle(0, 0,  radius=50)
    octagon = shape.Polygon(0, 0, radius=50, num_sides=8)
    hexagon = shape.Polygon(0, 0, radius=50, num_sides=6)
    pentagon = shape.Polygon(0, 0, radius=50, num_sides=5)
    square = shape.Polygon(0, 0, radius=50, num_sides=4)
    triangle = shape.Polygon(0, 0, radius=50, num_sides=3)

    # ADDING TEST
    #result = octagon - circle - hexagon + pentagon + pentagon - square - triangle - triangle

    for n in range(N + 1):
        theta = n/N * 2 * pi

        octagon_pt = octagon.get_point(theta)
        octagon_points.append(octagon_pt)

        hexagon_pt = hexagon.get_point(theta)
        hexagon_points.append(hexagon_pt)

        pentagon_pt = pentagon.get_point(theta)
        pentagon_points.append(pentagon_pt)

        square_pt = square.get_point(theta)
        square_points.append(square_pt)

        triangle_pt = triangle.get_point(theta)
        triangle_points.append(triangle_pt)
    ...
    shape_data = [
            (1, 1, 1, circle_points),
            (0, 1, 1, triangle_points),
            (1, 1, 0, square_points),
            (1, 0, 1, pentagon_points),
            (.75, .75, 1, hexagon_points),
            (1, .75, .75, octagon_points),
            ]
    for i in range(len(circle_points)//100):
        i = i * 100
