...
def draw_frame(N, circle1, circle2):
    circle1_points = []
    circle2_points = []
    #result_points = []
    ...
    for n in range(N + 1):
        ...
        circle1_pt = circle1.get_point(theta)
        circle1_points.append(circle1_pt)

        circle2_pt = circle2.get_point(theta)
        circle2_points.append(circle2_pt)

        # result_pt = result.get_point(theta)
        # result_points.append(result_pt)

    return [
                (1, .75, .5, circle1_points),
                (.5, .75, 1, circle2_points),
           ]
...
def makeImage(height, width, filename):
    ...
    circle1 = shape.Circle(0, 0,  100)
    circle2 = shape.Circle(300, 0, 50)
    ...
    for offset_num in range(offsets + 1):
        ...
        frame_points = draw_frame(N, circle1.rotate(7 * theta), 
                                     circle2.rotate(-3 * theta, 0, 0))
