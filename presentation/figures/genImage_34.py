...
def draw_frame(N, circle, square, triangle):
    circle_points = []
    square_points = []
    triangle_points = []
...
def makeImage(height, width, filename):
    ...
    circle = shape.Circle(0, 0,  100)
    square = shape.Polygon(0, 0, 50, 4)
    triangle = shape.Polygon(200, 200, 10, 3) 
    ...
    for offset_num in range(offsets + 1):
        ...
        frame_points = draw_frame(N, 
                                  circle.rotate(-5 * theta, 100, 0),
                                  square.rotate(7 * theta, -100,0),
                                  triangle.rotate(2 * theta, 300, 200))
